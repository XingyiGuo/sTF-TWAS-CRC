...
#load sim_y files
sim_y=fread(paste0(wk, "/simulation_values_res/sim_y_values/beta1_",beta1,"_h2_",h2,"_SNPs_",snps,".sim_y.txt"))
sim_y_names=colnames(sim_y)[1:length(colnames(sim_y))]

#load glm res
glm_res=as.data.frame(fread(paste0(wk,"/simulation_values_res/lmer_res/beta1_1_h2_",h2,"_SNPs_",snps,".sim_y.lmer.csv")))
glm_res = glm_res[!is.na(glm_res$CistromeID), ]
glm_res$CistromeID <- as.character(glm_res$CistromeID)

#load ldsc res
format_ldsc <- function(beta1) {
  ldsc_fd=paste0(wk,"/simulation_values_res/ldsc_res/y_sim_h2_",h2,"_SNPs_",snps)
  files=dir(ldsc_fd)
  ...
  for(file in files){
    ...
    lines<-readLines(paste0(wk,"/simulation_values_res/ldsc_res/y_sim_h2_",h2,"_SNPs_",snps,"/", file))
...
fwrite(out, paste0(wk,"/simulation_values_res/lmer_ldsc_beta1_1_h2_",h2,"_SNPs_",snps,".sim_y.lmer.csv"))
